...
SkyBox::SkyBox(std::vector<std::string> face_locations)
{
	...
	glErrorChecker.arePreError("From SkyBox constructor in SkyBox.cpp file.");
	...
	if (glErrorChecker.areErrorPrintAll("From SkyBox constructor in SkyBox.cpp file.")) {
		// return false;
	}
	...
	std::vector<Vertex> sky_box_vertices = {
		...
		Vertex(glm::vec3(-1.0f, 1.0f, -1.0f), glm::vec3(0.0f,0.0f,0.0f), glm::vec2(0.0f, 0.0f)),
		Vertex(glm::vec3(-1.0f, -1.0f, -1.0f), glm::vec3(0.0f,0.0f,0.0f), glm::vec2(0.0f, 0.0f)),
		Vertex(glm::vec3(1.0f, 1.0f, -1.0f), glm::vec3(0.0f,0.0f,0.0f), glm::vec2(0.0f, 0.0f)),
		Vertex(glm::vec3(1.0f, -1.0f, -1.0f), glm::vec3(0.0f,0.0f,0.0f), glm::vec2(0.0f, 0.0f)),
...
{
	// check if there any other gl Error  appears before execue gl functions
	glErrorChecker.arePreError("From draw_sky_box function in SkyBox.cpp file.");
	...
	// check with glErrorChecker.h if they are any glError and print it.
	if (glErrorChecker.areErrorPrintAll("From draw_sky_box function in SkyBox.cpp file.")) {
		// do something?
	}
