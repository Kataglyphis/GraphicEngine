...
Texture::Texture()
{
	...
	this->wrapping_mode = new RepeatMode();
}

Texture::Texture(const char* file_loc, TextureWrappingMode* wrapping_mode)
{
	textureID = 0;
	width = 0;
	height = 0;
...
bool Texture::load_texture_without_alpha_channel()
{
	...
	glErrorChecker.arePreError("From load_texture_without_alpha_channel in Texture.cpp file.");
...
{
	// check if they are errors before the function is executed
	glErrorChecker.arePreError("From load_texture_with_alpha_channel in Texture.cpp file.");
	stbi_set_flip_vertically_on_load(true);
	unsigned char* texture_data = stbi_load(file_location.c_str(), &width, &height, &bit_depth, 0);
	if (!texture_data) {
		printf("Failed to find: %s\n", file_location);
...
void Texture::use_texture()
{
	...
	// check with glErrorChecker.h if they are any glError and print it.
	if (glErrorChecker.areErrorPrintAll("From use_texture in Texture.cpp file.")) {
		// return false;
	}
}
