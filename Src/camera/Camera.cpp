...
Camera::Camera() :
    ...
    front(glm::vec3(0.0f, 0.0f, -1.0f)), world_up(glm::vec3(0.0f, 1.0f, 0.0f)), right(glm::normalize(glm::cross(front, world_up))),
    up(glm::normalize(glm::cross(right, front))), yaw(-60.0f), pitch(0.0f), movement_speed(35.0f), turn_speed(0.25f), near_plane(0.1f), far_plane(1000.f),
    fov(45.f)
...
Camera::Camera(glm::vec3 start_position, glm::vec3 start_up, GLfloat start_yaw, GLfloat start_pitch, GLfloat start_move_speed, GLfloat start_turn_speed,
  GLfloat near_plane, GLfloat far_plane, GLfloat fov) :
    ...
    //here we want the normal coord. axis z is showing to us !!
    front(glm::vec3(0.0f, 0.0f, -1.0f)), world_up(start_up), right(glm::normalize(glm::cross(front, world_up))), up(glm::normalize(glm::cross(right, front))),
    yaw(start_yaw), pitch(start_pitch), movement_speed(start_move_speed), turn_speed(start_turn_speed), near_plane(near_plane), far_plane(far_plane), fov(fov)
...
void Camera::set_near_plane(GLfloat near_plane) { this->near_plane = near_plane; }
