...
void Noise::create_shader_programs() {
  ...
  texture_1_shader_program->create_computer_shader_program_from_file(
      "clouds/noise_texture_128_res.comp");
  texture_2_shader_program->create_computer_shader_program_from_file(
      "clouds/noise_texture_32_res.comp");
}
...
void Noise::generate_num_cells_textures() {
  ...
  for (int i = 0; i < NUM_CELL_POSITIONS; i++) {
    ...
    glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA32F, num_cells_per_axis[i],
                 num_cells_per_axis[i], num_cells_per_axis[i], 0, GL_RGBA,
                 GL_FLOAT, cell_data[i].data());
...
void Noise::generate_res128_noise_texture() {
  ...
  glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA32F, texture_dim_1, texture_dim_1,
               texture_dim_1, 0, GL_RGBA, GL_FLOAT, NULL);

  glBindImageTexture(NOISE_128D_IMAGE_SLOT, texture_1_id, 0, GL_FALSE, 0,
...
void Noise::generate_res32_noise_texture() {
  ...
  glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA32F, texture_dim_2, texture_dim_2,
               texture_dim_2, 0, GL_RGBA, GL_FLOAT, NULL);

  glBindImageTexture(NOISE_32D_IMAGE_SLOT, texture_2_id, 0, GL_FALSE, 0,
                     GL_READ_WRITE, GL_RGBA32F);
...
void Noise::generate_cells(GLuint num_cells_per_axis, GLuint cell_index) {
  cell_data[cell_index].reserve(num_cells_per_axis * num_cells_per_axis *
  ...
  for (int i = 0; i < static_cast<int>(num_cells_per_axis); i++) {
    ...
    for (int k = 0; k < static_cast<int>(num_cells_per_axis); k++) {
      ...
      for (int m = 0; m < static_cast<int>(num_cells_per_axis); m++) {
        ...
        GLfloat position[3] = {(m + random_offset[0]), (k + random_offset[1]),
                               (i + random_offset[2])};
...
void Noise::create_res128_noise() {
  ...
  for (uint32_t i = 0; i < NUM_CELL_POSITIONS; i++) {
    ss << "cell_positions[" << i << "]";
    texture_1_shader_program->setUniformInt(NOISE_CELL_POSITIONS_SLOT + i,
                                            ss.str());
    ss.clear();
    ss.str(std::string());
  ...
  glDispatchCompute((GLuint)texture_dim_1, (GLuint)texture_dim_1,
...
void Noise::create_res32_noise() {
  ...
  for (uint32_t i = 0; i < NUM_CELL_POSITIONS; i++) {
    ss << "cell_positions[" << i << "]";
    texture_2_shader_program->setUniformInt(NOISE_CELL_POSITIONS_SLOT + i,
                                            ss.str());
    ss.clear();
    ss.str(std::string());
  ...
  glDispatchCompute((GLuint)texture_dim_2, (GLuint)texture_dim_2,
