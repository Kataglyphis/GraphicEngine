...
Clouds::Clouds() :
    ...
    shader_program(std::make_shared<ShaderProgram>(ShaderProgram{})), noise(std::make_shared<Noise>()), random_numbers(std::make_shared<RandomNumbers>()),
    model(glm::mat4(1.f)), aabb(AABB()),
...
void Clouds::set_num_march_steps_to_light(GLuint num_march_steps_to_light) { this->num_march_steps_to_light = num_march_steps_to_light; }
