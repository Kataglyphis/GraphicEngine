...
void DirectionalShadowMapPass::execute(
  glm::mat4 projection, std::shared_ptr<Camera> main_camera, GLuint window_width, GLuint window_height, std::shared_ptr<Scene> scene)
{

  std::shared_ptr<DirectionalLight> sun = scene->get_sun();
  //retreive shadow map before our geometry pass
  sun->calc_orthogonal_projections(main_camera->get_viewmatrix(), main_camera->get_fov(), window_width, window_height, NUM_CASCADES);
...
void DirectionalShadowMapPass::create_shader_program()
{
  ...
  shader_program->create_from_files(
    "rasterizer/shadows/directional_shadow_map.vert", "rasterizer/shadows/directional_shadow_map.geom", "rasterizer/shadows/directional_shadow_map.frag");
}
