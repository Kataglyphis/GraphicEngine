...
DirectionalLight::DirectionalLight()
    :
      ...
      shadow_near_plane(0.f),
      shadow_far_plane(0.f),
...
DirectionalLight::DirectionalLight(GLuint shadow_width, GLuint shadow_height,
                                   ...
                                   GLfloat radiance, GLfloat x_dir,
                                   GLfloat y_dir, GLfloat z_dir,
...
glm::mat4 DirectionalLight::get_light_view_matrix() const {
  return glm::lookAt(direction, glm::vec3(0.0f, 0.0f, 0.0f),
...
void DirectionalLight::update_shadow_map(GLfloat shadow_width,
...
void DirectionalLight::calc_cascaded_slots() {
  ...
  for (int i = 0; i < static_cast<int>(number_of_elements + 1); i++) {
    if (i == 0) {
      ...
      (cascade_slots)[i] =
          (shadow_far_plane) * ((GLfloat)i / (GLfloat)(number_of_elements));
    }
  }
...
void DirectionalLight::calc_orthogonal_projections(
    glm::mat4 camera_view_matrix, GLfloat fov, GLuint window_width,
  ...
  for (int i = 0; i < static_cast<int>(current_num_cascades); i++) {
    glm::mat4 curr_cascade_proj = glm::perspective(
        glm::radians(fov), (float)window_width / (float)window_height,
        cascade_slots[i], cascade_slots[i + 1]);
