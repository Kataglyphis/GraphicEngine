...
SkyBox::SkyBox() {
  ...
  std::array<std::string, 6> skybox_textures = {"DOOM16RT.png", "DOOM16LF.png",
                                                "DOOM16UP.png", "DOOM16DN.png",
                                                "DOOM16FT.png", "DOOM16BK.png"};
  ...
  for (size_t i = 0; i < 6; i++) {
    unsigned char* texture_data =
        stbi_load(skybox_faces[i].c_str(), &width, &height, &bit_depth, 0);
    if (!texture_data) {
...
void SkyBox::draw_sky_box(glm::mat4 projection_matrix, glm::mat4 view_matrix,
                          GLuint window_width, GLuint window_height,
                          GLfloat delta_time) {
  // https://learnopengl.com/Advanced-OpenGL/Cubemaps
  GLfloat velocity = movement_speed * delta_time;
  shader_playback_time =
      static_cast<GLfloat>(fmod(shader_playback_time + velocity, 10000));
...
void SkyBox::reload() {
  shader_program = std::make_shared<ShaderProgram>();
  shader_program->create_from_files("Shaders/SkyBox.vert",
                                    "Shaders/SkyBox.frag");
}
