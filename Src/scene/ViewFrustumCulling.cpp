...
bool ViewFrustumCulling::is_inside(GLfloat ratio,
  ...
  GLint outcodes_pattern[NUM_FRUSTUM_PLANES] = {
      outcode_near_plane, outcode_far_plane, outcode_up,
      outcode_bottom,     outcode_left,      outcode_right};
...
bool ViewFrustumCulling::corners_outside_plane(
    std::vector<glm::vec3> aabb_corners, frustum_plane plane,
...
GLfloat ViewFrustumCulling::plane_point_distance(frustum_plane plane,
                                                 glm::vec3 corner) {
  GLfloat result = 0.0f;
...
void ViewFrustumCulling::update_frustum_param(
    GLfloat near_plane, GLfloat far_plane, GLfloat fov, GLfloat ratio,
    std::shared_ptr<Camera> main_camera) {
  ...
  near_center = main_camera->get_camera_position() +
                main_camera->get_camera_direction() * near_plane;
  far_center = main_camera->get_camera_position() +
               main_camera->get_camera_direction() * far_plane;
...
void ViewFrustumCulling::init(std::vector<glm::vec3> frustum_corner) {
  ...
  float vertices[] = {
      ...
      frustum_corner[0].x, frustum_corner[0].y,
      frustum_corner[0].z,  // left bottom front
      frustum_corner[1].x, frustum_corner[1].y,
      frustum_corner[1].z,  // left bottom back
      frustum_corner[2].x, frustum_corner[2].y,
      frustum_corner[2].z,  // left top front
      frustum_corner[3].x, frustum_corner[3].y,
      frustum_corner[3].z,  // left top back
      frustum_corner[4].x, frustum_corner[4].y,
      frustum_corner[4].z,  // right bottom front
      frustum_corner[5].x, frustum_corner[5].y,
      frustum_corner[5].z,  // right bottom back
      frustum_corner[6].x, frustum_corner[6].y,
      frustum_corner[6].z,  // right top front
      frustum_corner[7].x, frustum_corner[7].y,
